...
function RollingLabel({ label }: RollingLabelProps) {
  return (
    <span className="grid h-7 overflow-hidden text-sm leading-7">
      <motion.span
      ...
      >
        <span className="flex h-7 items-center justify-center">{label}</span>
...
function PillFace({ label, className }: PillFaceProps) {
  return (
    <motion.div
      className={cn(
        "flex h-12 w-full shrink-0 items-center justify-center uppercase",
        className
      )}
      initial="rest"
      animate="rest"
...
function PillButton({
  ...
  return (
    <div
      ...
      className={cn(
        "relative h-12 w-20 cursor-pointer overflow-hidden rounded-full",
