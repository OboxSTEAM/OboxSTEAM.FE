...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const itemClassName =
    "sm-panel-item relative text-black font-semibold text-[1.85rem] sm:text-[2.3rem] cursor-pointer tracking-[-0.5px] uppercase transition-[background,color] duration-150 ease-linear flex items-center justify-between gap-6 w-full text-left bg-transparent border-0 no-underline";

  const renderItemContent = (item: StaggeredMenuItem) => {
    const Icon = item.icon;
    return (
      <>
        <span className="sm-panel-itemLabel inline-block min-w-0 flex-1 [transform-origin:50%_100%] will-change-transform">
          {item.label}
        </span>
        {Icon ? (
          <Icon
            className="sm-panel-itemIcon size-7 shrink-0 text-[var(--sm-accent,#E94B3C)]"
            strokeWidth={2}
            aria-hidden="true"
          />
        ) : null}
...
.sm-scope .sm-panel-item { position: relative; color: #000; font-weight: 600; cursor: pointer; line-height: 1.3; letter-spacing: -0.5px; text-transform: uppercase; transition: background 0.25s, color 0.25s; display: flex; align-items: center; justify-content: space-between; gap: 1.5rem; text-decoration: none; padding-bottom: 0.08em; }
.sm-scope .sm-panel-itemLabel { display: inline-block; will-change: transform; transform-origin: 50% 100%; }
.sm-scope .sm-panel-itemIcon { flex-shrink: 0; opacity: 0.85; transition: opacity 0.25s, color 0.25s; }
